...
const CheckUser = async (req,res) => {
    ...
    if( !usermail || !pwd) return res.status(400).json({'message' : 'Username and Password are required. '});
    ...
    try{
        ...
        if(!matchpwd && pwd !== founder.password) return res.status(409).json({'message': 'Password Incorret'});
        else {
